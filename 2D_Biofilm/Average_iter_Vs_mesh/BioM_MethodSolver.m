function [ M_iter_per_step_values, h_M_x_values, EA ] =  BioM_MethodSolver(Lx, Rx, Ly, Ry, nx_values, ny_values, T, dt, ee, m)
...
for nx_index = 1:length(nx_values)
    ...
    for i = t_start:t_end
        ...
        for V = 1:iteration_no 
            b_iter = Biobs(S_iter); % This is the value of b at S^{i-1}_n
            B_iter = BioBBs(S_iter); % This is the value of B at S^{i-1}_n
            dbds_iter = Biodbds(S_iter); % This is the value of b' at S^{i-1}_n
            dBds_iter = BiodBBds(S_iter); % This is the value of B' at S^{i-1}_n
            ...
            for ind = 1:(nx + 1) * (ny + 1)
                ...
                if j == 0 || j == 1 || k == 1 || k == ny + 1
                    ...
                    Lmat(ind, ind) = (4 * (dtdx)) + (Lb(j, k) ./ LB(j, k)) .* (1 - ee * (1 - b_n(j, k)) * dt);
                    Lmat(ind, ind - 1) = -(dtdx);
                    Lmat(ind, ind + 1) = -(dtdx);
                    Lmat(ind, ind - nx - 1) = -(dtdx);
                    Lmat(ind, ind + nx + 1) = -(dtdx);
                    f(ind) = b_n(j, k) + (1 - ee * (1 - b_n(j, k)) * dt) .* ((Lb(j, k) ./ LB(j, k)) .* B_iter(j, k) - b_iter(j, k));
                end
            end
