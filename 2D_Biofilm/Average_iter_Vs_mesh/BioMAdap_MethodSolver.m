function [MAdap_iter_per_step_values, h_MAdap_x_values] = BioMAdap_MethodSolver(Lx, Rx, Ly, Ry, nx_values, ny_values, T, dt, ee, m)
...
for nx_index = 1:length(nx_values)
    ...
    for i = t_start:t_end
        ...
        b_iter = Biobs(S_iter); 
        B_iter = BioBBs(S_iter); 
        dbds_iter = Biodbds(S_iter); 
        dBds_iter = BiodBBds(S_iter);

        for V = 1:iteration_no
            %% Definition of Lb and LB (M-scheme)
