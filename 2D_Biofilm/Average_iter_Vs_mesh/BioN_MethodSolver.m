function [N_iter_per_step_values, h_Nx_values] = BioN_MethodSolver(Lx, Rx, Ly, Ry, nx_values, ny_values, T, dt, ee, m)
            ...
            for V = 1:iteration_no
                ...
                for ind = 1:(nx + 1) * (ny + 1)
                    ...
                    if j == 0 || j == 1 || k == 1 || k == ny + 1
                        ...
                        Lmat(ind, ind) = 4 * LB(j, k) * (dt / dx ^ 2) + Lb(j, k) * (1 - ee * (1 - b_n(j, k)) * dt);
                        ...
                        F = b_n(j, k) + LB(j, k) * (2 * dt / dx ^ 2) * S_iter(j, k) - 2 * (dt / dx ^ 2) * B_iter(j, k) - ...
                            ...
                            (1 - ee * (1 - b_n(j, k)) * dt) * (Lb(j, k) * S_iter(j, k) - b_iter(j, k));
                        f(ind) = F;
                    end
                end
