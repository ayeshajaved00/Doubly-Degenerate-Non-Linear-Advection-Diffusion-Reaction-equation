function [M_iter_per_step_values, h_M_x_values] = MR_MethodSolver(Lx, Rx, Ly, Ry, nx_values, ny_values, b_values, B_values, T, dt, ee, m)
...
for nx_index = 1:length(nx_values)
    ...
    for i = t_start:t_end
        ...
        for V = 1:iteration_no
            ...
            b_iter = Rbs(S_iter, index, b_values);
            B_iter = RBB(S_iter, index, B_values);
            dbds_iter = Rdbds(S_iter, index, b_values);
            dBds_iter = RdBBds(S_iter, index, B_values);
            ...
            for ind = 1:(nx + 1) * (ny + 1)
                ...
                if j == 0 || j == 1 || k == 1 || k == ny + 1
                    ...
                    f(ind) = b_n(j, k) + (1 - ee * dt) * ((Lb(j, k) / LB(j, k)) * B_iter(j, k) - b_iter(j, k)) + ...
                             (dt / dx) * (-2 * F_iter(j, k) + F_iter(j - 1, k) + F_iter(j, k - 1));
                end
            end
