%% DD Toy-model - Average iteration vs Mesh size (h)
% This script solves the DD Toy model using iterative 
% scheme (M scheme)

function [M_iter_per_step_values, h_M_values] = M_MethodSolver(L, R, n_values, T, dt, ee, m)

    % Initialize arrays to store iter_per_step and h values for each n
    M_iter_per_step_values = zeros(size(n_values));
    h_M_values = zeros(size(n_values));

    MB = 0.01;
    Mb = 0.01;

    for n_index = 1:length(n_values)
        n = n_values(n_index);
        dx = (R - L) / n;
        h = 1 / dx;

        % Generate spatial grid points
        x = zeros(1, n + 1);
        for i = 1:n + 1
            x(i) = L + dx * (i - 1);
        end

        % Number of time steps
        N = floor(T / dt) + 1;  
        t = zeros(1, N);

        % Boundary conditions
        Pl = 0;
        Pr = 0;

        % Initial condition
        S0 = InitialWW(x, m, 0, 1);
        b_n = S0;
        S_current = S0;

        % Time loop initialization
        t_start = 1;
        t_end = N;
        diverge = 0;
        iteration_no = 50; 
        tot_iter_number = 0;

        for j = t_start:t_end
            t(j) = (j - 1) * dt;
            EA = [];
            S_iter = S_current;

            % Precomputing arrays for the matrix
            tm = ((dt / dx^2)) .* ones(1, n - 1);
            tp = ((dt / dx^2)) .* ones(1, n - 1);
            Main_D = [1 (tm + tp) 1];
            Lower_D = [0 -tm 0];
            Upper_D = [0 -tp 0];

            for i = 1:iteration_no
                % Compute b, B, db/ds, and dB/ds at the current iteration
                b_iter = bs(S_iter);       % Value of b at S^{i-1}_n
                B_iter = BBs(S_iter);      % Value of B at S^{i-1}_n
                dBds_iter = dBBds(S_iter); % Value of B' at S^{i-1}_n
                dbds_iter = dbds(S_iter);  % Value of b' at S^{i-1}_n

                % Define Lb and LB
                Lb = min(max(dbds_iter + Mb * dt, 2 * Mb * dt), 1);
                LB = min(max(dBds_iter + MB * dt, 2 * MB * dt), 1);

                % Compute the matrix and source term
                AA = Main_D + (Lb ./ LB) .* (1 - ee * dt);
                AA(1) = 1; 
                AA(end) = 1;

                f = b_n(2:n) - (1 - ee * dt) .* b_iter(2:n) + ...
                    (1 - ee * dt) .* B_iter(2:n) .* (Lb(2:n) ./ LB(2:n));

                F = [Pl f Pr];

                if i == 1
                    W_intiter = B_iter;
                else
                    W_intiter = W_iter;
                end

                % Solve the tridiagonal system
                W_iter = tridiagQQ(AA, Lower_D, Upper_D, F)';

                % Update S_iter
                S_intiter = S_iter;

                S_iter = (1 ./ LB) .* (W_iter - B_iter) + S_intiter;

                %% Error calculation
                dW_dx = gradient(W_iter - W_intiter, dx);
                               
                err = sqrt(trapz(x,Lb .* LB .*  (S_iter - S_intiter).^2 + dt * (dW_dx.^2)));

                EA = [EA, err];

                tot_iter_number = tot_iter_number + 1;

                %% Convergence check
                if err < 1e-6
                    break;
                elseif isnan(err) || err > 100
                    diverge = 1;
                    break;
                end
            end

            % Update variables for the next time step
            S_current = S_iter;
            b_n = bs(S_current);

            if diverge == 1
                iter_per_timestep = inf;
                break;
            end
        end

        if diverge ~= 1
            iter_per_timestep = tot_iter_number / t_end;
        end

        M_iter_per_step_values(n_index) = iter_per_timestep;
        h_M_values(n_index) = h;
    end
end
