...
function [MAdap_iter_per_step_values, h_MAdap_values,iteration_MAdap_iter, EA_MAdap, Mb] = MAdap_Error_MethodSolver(L, R, n_values, T, dt, ee, m)
    ...
    for n_index = 1:length(n_values)
        ...
        for j = t_start:t_end
            ...
            b_iter = bs(S_iter);      % b at S^{i-1}_n
            B_iter = BBs(S_iter);     % B at S^{i-1}_n
            dBds_iter = dBBds(S_iter); % B' at S^{i-1}_n
            dbds_iter = dbds(S_iter);  % b' at S^{i-1}_n
            ...
            for i = 1:iteration_no      
                ...
                elseif isnan(err) || err > 100
                    fprintf('Divergence detected MAdap %d\n', i);
                    break; 
                end
