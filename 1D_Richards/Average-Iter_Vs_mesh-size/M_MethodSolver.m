%% Richards Equation - Average iteration Vs mesh size (h)
% This script solves the Richards equation using iterative 
% scheme (M-scheme) and visualizes the Average iteration Vs mesh size (h)

function [M_iter_per_step_values, h_M_values] = M_MethodSolver(L, R, n_values, b_values, B_values, T, dt, ee, m)
    global Ks
    global lambda
   
    M = 0.01;
    
    %% Initialize an array to store iter_per_step for each n
    M_iter_per_step_values = zeros(size(n_values));
    h_M_values = zeros(size(n_values));
    iteration_M_iter = zeros(size(n_values));
    
    for n_index = 1:length(n_values)
        n = n_values(n_index);
        dx = (R-L)/n;
        h = 1 / dx;
        h_M_values(n_index) = h;
        x = zeros(1, n+1);
        
        for i = 1:n+1
            x(i) = L + dx * (i - 1);
        end
        
        %% The Number of Time Steps
        N = floor(T/dt) + 1;
        t = zeros(1, N);
        
        %% Boundary Condition
        Pl = 0;
        Pr = 0;
        
        %% Initial condition
        S0 = InitialWW(x, m, 0, 1);
        b_n = S0;
        S_current = S0;
        
        %% Basic iteration information
        t_start = 1;
        t_end = N;
        diverge = 0;
        iteration_no = 50;
        
        %% Start of time loop  
        tot_iter_number = 0;
        
        for j = t_start:t_end
            t(j) = (j - 1) * dt;
            EA_M = [];
            S_iter = S_current;
            
            %% Precomputing the arrays
            tm = ((dt/dx^2)) .* ones(1, n-1);
            tp = ((dt/dx^2)) .* ones(1, n-1);
            Main_D = [1 (tm + tp) 1];
            Lower_D = [0 -tm 0];
            Upper_D = [0 -tp 0];
            
            for i = 1:iteration_no
                S_iter = max(S_iter, 0);
                index = idx(S_iter, b_values);
                b_iter = bs(S_iter, index, b_values);
                B_iter = BB(S_iter, index, B_values);
                dbds_iter = dbds(S_iter, index, b_values);
                dBds_iter = dBBds(S_iter, index, B_values);
                F_iter = Ks(b_iter, lambda);
                
                %% Defining LB and Lb
                Lb = min(max(dbds_iter + M * dt, 2 * M * dt), 1);
                LB = min(max(dBds_iter + M * dt, 2 * M * dt), 1);
                
                AA = Main_D + (Lb ./ LB) .* (1 - ee * dt);
                AA(1) = 1;
                AA(end) = 1;
                
                f = b_n(2:n) - (1 - ee * dt) .* b_iter(2:n) + (1 - ee * dt) .* B_iter(2:n) .* ((Lb(2:n)) ./ (LB(2:n))) + (dt/dx) .* (F_iter(1:n-1) - F_iter(2:n));
                F = [Pl f Pr];
                
                if i == 1
                   W_intiter = B_iter;
                else
                   W_intiter = W_iter;
                end
                
                W_iter = tridiagQQ(AA, Lower_D, Upper_D, F)';
                S_intiter = S_iter;
                S_iter = (1 ./ LB) .* (W_iter - B_iter) + S_intiter;
                
                %% Error check
                dW_dx = gradient(W_iter - W_intiter, dx);
                               
                err = sqrt(trapz(x, Lb .* LB .*(S_iter - S_intiter).^2 + dt *(dW_dx.^2)));

                EA_M = [EA_M, err];
                tot_iter_number = tot_iter_number + 1;
                
                %% Convergence check
                if err < 1e-6
                    break;
                elseif isnan(err) || err > 100
                    diverge = 1;
                    break;
                end
            end
            
            %% Variable Update
            S_current = S_iter;
            b_n = bs(S_current, index, b_values);
            EA_M;
            
            if diverge == 1
                iter_per_timestep = inf;
                break;
            end
        end
        
        if diverge ~= 1
            iter_per_timestep = tot_iter_number / t_end;
        end
        M_iter_per_step_values(n_index) = iter_per_timestep;
        iteration_M_iter(n_index) = i;
    end
end